...
GEMINI_API_KEY = "AIzaSyBUuEMlPfisln_gEs5szqdQe5ANhPZ9-oU"  # Your Gemini API key
...
if menu == "Chat with AI":
    ...
    if st.button("Get Diagnosis"):
        if user_input:
            try:
                # Initialize the Gemini model with fixed gemini-1.5-pro
...
# ---- UPLOAD REPORTS ----
    ...
    if uploaded_report is not None:
        try:
            ...
            # Analyze with Gemini using fixed gemini-1.5-pro
            model = genai.GenerativeModel("gemini-1.5-pro")
            prompt = (
