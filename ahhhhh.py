...
MODEL_PATH_2 = "D:\A2vp\The_model.h5"
model_1 = tf.keras.models.load_model(MODEL_PATH_1)
model_2 = tf.keras.models.load_model(MODEL_PATH_2)

# Define the class labels for the second model
classes = ["brownwarts", "Carsinoma", "fibrous hyst", "Melanoma", "hemorrage", "Mole", "bouwen disease"]
...
if menu == "Chat with AI":
    ...
    st.title("📸 Upload Medical Images for Diagnosis (Model 2)")
    uploaded_file = st.file_uploader("Upload an Image", type=["png", "jpg", "jpeg"])
    if uploaded_file is not None:
        file_bytes = np.asarray(bytearray(uploaded_file.read()), dtype=np.uint8)
        image = cv2.imdecode(file_bytes, cv2.IMREAD_COLOR)
        ...
        # st.write("Raw Predictions:", predictions_2.tolist())
